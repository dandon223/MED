...
def test() -> None:
    ...
    #test_rand_score_time(artificial, results_folder + "single_rand_score_artificial-subset.csv", "euclidean", "single", 5)
    print("single_test_rand_score_real_world_subset")
    #test_rand_score_time(real_world, results_folder + "single_rand_score_real_world_subset.csv", "euclidean", "single", 5)

    print("complete_test_rand_score_artificial")
    #test_rand_score_time(artificial, results_folder + "complete_rand_score_artificial-subset.csv", "euclidean", "complete", 5)
    print("complete_test_rand_score_real_world_subset")
    #test_rand_score_time(real_world, results_folder + "complete_rand_score_real_world_subset.csv", "euclidean", "complete", 5)

    print("average_test_rand_score_artificial")
    #test_rand_score_time(artificial, results_folder + "average_score_artificial-subset.csv", "euclidean", "average", 5)
    print("average_test_rand_score_real_world_subset")
    #test_rand_score_time(real_world, results_folder + "average_score_real_world_subset.csv", "euclidean", "average", 5)

    print("single_manhattan_test_rand_score_artificial")
    #test_rand_score_time(artificial, results_folder + "single_manhattan_rand_score_artificial-subset.csv", "manhattan", "single", 5)
    print("single_manhattan_test_rand_score_real_world_subset")
    #test_rand_score_time(real_world, results_folder + "single_manhattan_rand_score_real_world_subset.csv", "manhattan", "single", 5)
    ...
    #test_number_of_rows(rows_number, results_folder + "letter_rows_time.csv", number_of_times=2)


    print("type_of_algorithm_test")
    type_of_algorithm_test(artificial, results_folder + "type_of_algorithm_time.csv", 2, "single")
    return 0
...
def test_number_of_rows(rows_number: int,csv_name:str, dataset_name: str = "../datasets/real-world/letter.arff", number_of_times: int=5, algorithm: str="euclidean", formula: str="single"):
...
def test_number_of_features(columns_number, csv_name:str, dataset_name: str = "../datasets/real-world/arrhythmia.arff", number_of_times=5, algorithm="euclidean", formula="single"):
    data, number_of_clusters = get_dataset(dataset_name)
    Y = data['ground_truth']
...
def run_nn_linkage_clustering(data: pd.DataFrame, number_of_clusters: int, algorithm: str, formula: str ) -> Tuple[float, float, pd.Series]:
    ...
    data[NN_LINKAGE_CLUSTERING] = np.nan
    for index, cluster in enumerate(clusters):
        for point in cluster:
            data.at[point, NN_LINKAGE_CLUSTERING] = index
    end = time.time()
    score = metrics.rand_score( labels_true=Y_data, labels_pred=data[NN_LINKAGE_CLUSTERING])
    time_run = end - start
    return score, time_run, data[NN_LINKAGE_CLUSTERING]
...
def dev_test(dataset: str = "../datasets/artificial/target.arff") -> None: # artificial/target # real-world/balance-scale.arff
    data, number_of_clusters = get_dataset(dataset)
    score, time_run, labels = run_agglomerative_clustering(data, number_of_clusters, 'euclidean', 'single')

    data["AgglomerativeClustering"] = labels
    print('rand_score', score)
    print('time', time_run)
    
    score, time_run, labels = run_nn_linkage_clustering(data, number_of_clusters, 'euclidean', 'single')
    data['NNChainLinkageClustering'] = labels
    print('rand_score', score)
    print('time', time_run)
